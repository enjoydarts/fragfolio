...
export const LoginForm: React.FC<LoginFormProps> = ({
  ...
  const [twoFactorWebAuthnLoading, setTwoFactorWebAuthnLoading] = useState(false);
  const [twoFactorMethod, setTwoFactorMethod] = useState<'totp' | 'webauthn' | null>(null);
  ...
  useEffect(() => {
    if (requiresTwoFactor && twoFactorMethod === 'webauthn' && !twoFactorWebAuthnLoading && tempToken) {
  ...
  const handleTwoFactorSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8002'}/api/auth/two-factor-challenge`, {
        method: 'POST',
        headers: {
  ...
  const handleTwoFactorWebAuthn = async () => {
    if (!webAuthnSupported || !tempToken) {
      setError(t('auth.webauthn.not_supported', 'WebAuthnがサポートされていません'));
      return;
    }
    ...
    try {
      // 1. WebAuthnオプションを取得
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8002'}/api/auth/two-factor-webauthn`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      if (!data.success) {
        setError(data.message || t('auth.webauthn.login_failed', 'WebAuthnログインに失敗しました'));
        return;
      }

      // 2. WebAuthn API用に変換
      const requestOptions = WebAuthnUtils.convertLoginOptions(data.webauthn_options);
      ...
      if (!credential) {
        throw new Error(t('auth.webauthn.cancelled', '認証がキャンセルされました'));
      }
      ...
      // 5. サーバーに送信して2FA完了
      const completeResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8002'}/api/auth/two-factor-webauthn/complete`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
          ...
          <div className="bg-white/95 backdrop-blur-sm border border-white/20 rounded-2xl shadow-xl p-8">
            ...
            <div className="text-center mb-8">
              <h2 className="text-2xl font-light text-gray-800 mb-2">
                {requiresTwoFactor ? t('auth.two_factor.title', '2要素認証') : t('auth.login.title')}
              </h2>
              <p className="text-gray-500 text-sm font-light">
                {requiresTwoFactor ? t('auth.two_factor.subtitle', '認証コードを入力してください') : t('auth.login.subtitle')}
              </p>
            </div>

            <form className="space-y-5" onSubmit={requiresTwoFactor ? handleTwoFactorSubmit : handleSubmit}>
              {/* エラーメッセージ */}
              {error && (
                  ...
                  twoFactorMethod === null ? (
                    ...
                    <div className="space-y-4">
                      <div className="text-center mb-6">
                        <h3 className="text-lg font-medium text-gray-800 mb-2">
                          {t('auth.two_factor.choose_method', '認証方法を選択してください')}
                        </h3>
                        <p className="text-sm text-gray-600">
                          {t('auth.two_factor.choose_method_hint', 'どちらか一つの方法で認証を完了してください')}
                        </p>
                        {/* デバッグ情報 */}
                      ...
                      {/* WebAuthn選択ボタン */}
                      {availableMethods.includes('webauthn') && webAuthnSupported && (
                        <button
                          type="button"
                        ...
                        >
                          <div className="flex items-center justify-center">
                            ...
                            {t('auth.two_factor.webauthn_method', '生体認証（FIDO2）')}
                          </div>
                        </button>
                      )}
                    </div>
                    ...
                    <div className="group">
                      ...
                      <p className="text-xs text-gray-500 mt-2">
                        {t('auth.two_factor.code_hint', '認証アプリまたはリカバリコードを入力してください')}
                      </p>
                    </div>
                  ) : (
                    /* WebAuthn認証画面 */
                    <div className="text-center">
                      ...
                      <div className="bg-gray-50 rounded-xl p-6 mb-4">
                        ...
                        <p className="text-sm text-gray-600">
                          {t('auth.two_factor.webauthn_prompt', '生体認証器をタッチして認証を完了してください')}
                        </p>
                      </div>
                    </div>
                  )
                  ...
                  <label className="flex items-center group cursor-pointer">
                    <div className="relative">
                      <input
                        ...
                        onChange={handleChange}
                      />
                      <div className={`w-5 h-5 border-2 border-gray-300 rounded-md transition-all duration-200 ${formData.remember ? 'bg-amber-500 border-amber-500' : 'bg-white'}`}>
                        {formData.remember && (
                    ...
                    <span className="ml-3 text-sm text-gray-600 font-light">{t('auth.remember_me')}</span>
                  </label>
              ...
              <div className="pt-6 space-y-3">
                ...
                {(!requiresTwoFactor || twoFactorMethod === 'totp') && (
                  <button
                    type="submit"
                    disabled={loading || (!requiresTwoFactor && turnstileSiteKey && !turnstileToken) || (requiresTwoFactor && twoFactorMethod === 'totp' && !twoFactorCode)}
                  ...
                  >
                    {loading ? (
                      <div className="flex items-center justify-center">
                        ...
                        {requiresTwoFactor ? t('auth.two_factor.verifying', '認証中...') : t('auth.logging_in')}
                      </div>
                    ) : (
                      requiresTwoFactor && twoFactorMethod === 'totp' ? t('auth.two_factor.verify_button', '認証コードを確認') : t('auth.login.button')
                    )}
                  </button>
                )}
