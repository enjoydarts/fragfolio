...
export const TOTPSettings: React.FC = () => {
  ...
  const copyToClipboard = async (text: string) => {
    try {
      ...
      toast.success(
        t(
          'settings.security.twofa.code_copied',
          'リカバリコードをコピーしました'
        )
      );
    } catch (error) {
  ...
  const copyAllRecoveryCodes = async () => {
    try {
      ...
      toast.success(
        t(
          'settings.security.twofa.all_codes_copied',
          'すべてのリカバリコードをコピーしました'
        )
      );
    } catch (error) {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : t('settings.security.twofa.enable_failed');
      setState((prev) => ({
        ...prev,
        error: errorMessage,
        loading: false,
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await TwoFactorAPI.confirm(
        token,
      ...
      setState((prev) => ({
        ...
        error:
          error instanceof Error
            ? error.message
            : t('settings.security.twofa.verification_failed'),
        loading: false,
      }));
    }
  };
    ...
    return (
      <div className="border border-gray-200 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-600">
              {t(
                'settings.security.twofa.enabled_desc',
                '2要素認証が有効になっています'
              )}
            </p>
          </div>
        ...
        <div className="space-y-3">
          ...
          >
            {t(
              'settings.security.twofa.regenerate_recovery',
              'リカバリコードを再生成'
            )}
          </button>
          ...
          state.recoveryCodes.length > 0 && (
            <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              ...
              <div className="text-sm text-yellow-700 mb-3 space-y-2">
                <p>
                  {t(
                    'settings.security.twofa.recovery_desc',
                ...
                <div className="flex items-start gap-2 font-medium">
                  ...
                  <span>
                    {t(
                      'settings.security.twofa.recovery_usage_warning',
                      '重要: リカバリーコードを使用すると、セキュリティのため自動的に新しいコードが生成され、古いコードは無効になります。'
                    )}
                  </span>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 mb-3">
                {state.recoveryCodes.map((code, index) => (
                  <div
                    ...
                    title={t(
                      'settings.security.twofa.click_to_copy',
                      'クリックしてコピー'
                    )}
                  >
                    {code}
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() =>
                    setState((prev) => ({ ...prev, showRecoveryCodes: false }))
                  }
            ...
            <ConfirmDialog
              ...
              title={
                confirmDialog.type === 'disable'
                  ? t(
                      'settings.security.twofa.disable_confirm_title',
                      '2要素認証の無効化'
                    )
                  : t(
                      'settings.security.twofa.regenerate_confirm_title',
                      'リカバリコードの再生成'
                    )
              }
              message={
                confirmDialog.type === 'disable'
                  ? t(
                      'settings.security.twofa.disable_confirm',
                      '2要素認証を無効化しますか？セキュリティが低下する可能性があります。'
                    )
                  : t(
                      'settings.security.twofa.regenerate_confirm',
                      '新しいリカバリコードを生成しますか？古いコードは使用できなくなります。'
                    )
              }
              confirmText={
                confirmDialog.type === 'disable'
      ...
      {state.step === 'initial' && (
        <div>
          <p className="text-sm text-gray-600 mb-4">
            {t(
              'settings.security.twofa.description',
              'TOTPアプリを使用してアカウントのセキュリティを強化します'
            )}
          </p>
          <button
        ...
        <div className="space-y-4">
          <div>
            <h5 className="font-medium text-gray-900 mb-2">
              {t(
                'settings.security.twofa.setup_step1',
                'ステップ1: 認証アプリでQRコードをスキャン'
              )}
            </h5>
            <p className="text-sm text-gray-600 mb-3">
              {t(
                'settings.security.twofa.setup_apps',
                'Google Authenticator、Authy、1Passwordなどの認証アプリを使用してください'
              )}
            </p>
            {state.qrCode && (
          ...
          <div>
            <h5 className="font-medium text-gray-900 mb-2">
              {t(
                'settings.security.twofa.setup_step2',
                'ステップ2: 認証コードを入力'
              )}
            </h5>
      ...
      {state.step === 'complete' && (
        <div className="space-y-4">
          <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded">
            {t(
              'settings.security.twofa.enabled_success',
              '2要素認証が正常に有効化されました！'
            )}
          </div>
